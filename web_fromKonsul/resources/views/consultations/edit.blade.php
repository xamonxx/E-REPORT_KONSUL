...
<div class="max-w-3xl mx-auto px-1 sm:px-0">
    ...
    <div class="bg-surface-container-lowest rounded-2xl shadow-sm p-5 sm:p-8 border border-surface-container-low">
        ...
        <form method="POST" action="{{ route('consultations.update', $consultation) }}" class="space-y-6 sm:space-y-8">
            ...
            <div x-data="cityAutoFill(@js(old('city', $consultation->city ?? '')), @js(old('province', $consultation->province ?? '')))" class="space-y-5 sm:space-y-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5 sm:gap-6">
                    <div class="space-y-2">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5 sm:gap-6">
                <div class="space-y-2">
                    ...
                    <div x-data="searchableSelect(@js($categories->map(fn($category) => ['value' => (string) $category->id, 'label' => $category->name])->values()), @js(old('needs_category_id', $consultation->needs_category_id)))"
                         @click.outside="close()"
                         @keydown.escape.prevent.stop="close()"
                         class="relative">
